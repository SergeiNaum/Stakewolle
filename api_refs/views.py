...
from api_refs.serializers import (
    ReferralCodeSerializer,
    EmailSerializer,
    ReferralSerializer,
    ReferrerWithReferralsSerializer,
)
from api_refs.tasks import (
    create_referral_code_task,
    delete_referral_code_task,
    get_referral_list,
)
...
class ReferralCodeCreateView(generics.CreateAPIView):
    ...
    def post(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        active_referral_code: Optional[ReferralCode] = ReferralCode.objects.filter(
            user=user, expiration_date__gt=timezone.now()
        ).first()
        if active_referral_code:
            return Response(
                {"detail": "У пользователя уже существует активный реферальный код."},
                status=status.HTTP_400_BAD_REQUEST,
            )

        task = create_referral_code_task.delay(user.id)

        response_data: dict = {"message": "Реферальный код создан", "task_id": task.id}
        return Response(response_data, status=status.HTTP_202_ACCEPTED)


@extend_schema(tags=["Referral Codes"], summary="Destroy a user's referral code")
class ReferralCodeDestroyView(generics.DestroyAPIView):
    ...
    def delete(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        if active_referral_code:
            ...
            return Response(response_data, status=status.HTTP_202_ACCEPTED)
        return Response(
            {"error": "Реферальный код не найден"}, status=status.HTTP_404_NOT_FOUND
        )


@extend_schema(tags=["Referral Codes"], summary="Retrieve a referral code by email")
...
@extend_schema(
    tags=["Referral Codes"], summary="Show referal code for authenticated user"
)
...
@extend_schema(
    responses=ReferrerWithReferralsSerializer,
    tags=["Referrals"],
...
class ReferralListView(generics.GenericAPIView):
    ...
    def get(request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        referral_list_task = get_referral_list.delay(referrer_id)  # noqa F841

        serializer: ReferrerWithReferralsSerializer = ReferrerWithReferralsSerializer(
            user
        )
